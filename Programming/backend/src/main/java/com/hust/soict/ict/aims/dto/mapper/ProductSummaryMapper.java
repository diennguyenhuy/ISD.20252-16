...
abstract class ProductSummaryMapper<P extends Product> extends AbstractMapper<P, ProductSummary> {
    protected ProductSummaryMapper(Class<P> sourceType) {
        super(sourceType, ProductSummary.class, ProductSummary::new);
    }

    protected abstract List<String> mapCreators(P product);

    @Override
    protected final void map(P source, ProductSummary target) {
