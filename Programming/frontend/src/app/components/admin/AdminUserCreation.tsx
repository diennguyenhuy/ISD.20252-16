...
const inputClass = (err?: string) => {
    ...
    // The focus ring is properly set to destructive for the Admin theme
        ...
        <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground text-sm font-medium mt-1">Grant system access to new managers or
                        administrators. A temporary password will be emailed.</p>
                </div>
            </div>
            ...
            <div className="bg-card rounded-3xl border border-border shadow-lg p-6 sm:p-10">
                ...
                {apiError && (
                    <div
                        className="mb-6 p-4 bg-destructive/10 border border-destructive/20 text-destructive rounded-xl flex items-center gap-3 font-bold">
                        <AlertCircle size={20} className="shrink-0"/> {apiError}
                    </div>
                )}

                {/* Avatar Placeholder Preview - Already perfectly themed! */}
                <div className="flex items-center gap-5 mb-8 pb-8 border-b border-border/50">
                    <div
                        className="w-20 h-20 rounded-full bg-destructive/10 text-destructive border-2 border-destructive/20 flex items-center justify-center text-3xl font-extrabold shadow-inner shrink-0">
                    ...
                    <div className="sm:col-span-2">
                        <Field label="Username" required error={errors.username}>
                            <input value={form.username} onChange={update('username')}
                                   className={inputClass(errors.username)} placeholder="e.g., johndoe"/>
                        </Field>
                    </div>
                    ...
                    <div className="sm:col-span-2">
                        <Field label="Login Email" required error={errors.email}>
                            <input type="email" value={form.email} onChange={update('email')}
                                   className={inputClass(errors.email)} placeholder="e.g., admin@aims.vn"/>
                        </Field>
                    </div>
                    ...
                    <div className="sm:col-span-2">
                        <Field label="Roles" required error={errors.roles as any}>
                            <div className="flex flex-col sm:flex-row gap-4">
                                {[
                                    {
                                        value: 'PRODUCT_MANAGER' as UserRole,
                                    ...
                                    return (
                                        <label
                                            ...
                                            // Swapped primary with destructive for the selected border and background
                                        ...
                                        >
                                            <div className="mt-0.5">
                                                <input
                                                    type="checkbox"
                                                    // Swapped primary with destructive for the checkbox accent color
                                            ...
                                            <div>
                                                {/* Swapped primary with destructive for the selected text label */}
                                                <p className={`font-bold text-sm ${isSelected ? 'text-destructive' : 'text-foreground'}`}>
                                                    {roleOption.label}
                                                </p>
