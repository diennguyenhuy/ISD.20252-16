...
export default function AdjustStockModal({ product, onClose }: Props) {
    ...
    const validate = () => {
        ...
        if (delta === 0) errs.delta = 'Please enter an adjustment quantity (not 0)';
        if (newStock < 0) errs.delta = `Stock cannot be negative (current: ${product.stockQuantity})`;
        if (!reason.trim()) errs.reason = 'Please enter an adjustment reason';
        setErrors(errs);
    ...
    const handleSubmit = async () => {
        ...
        // Pass delta directly — hook sends it straight to AdjustStockRequest.delta
                ...
                {success ? (
                    ...
                    <div className="p-10 text-center animate-in fade-in zoom-in duration-300">
                        ...
                        <p className="text-sm font-medium text-muted-foreground mt-2">
                            New stock: <span className="text-primary font-bold text-base">{newStock}</span>
                        </p>
                    </div>
                ) : (
                    /* ── Form ── */
                    <div className="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-3">
                                <button
                                    ...
                                    className="w-12 h-12 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive flex items-center justify-center font-bold"
                                >
                                    <Minus size={20} />
                                </button>
                                <input
                                ...
                                <button
                                    onClick={() => { setDelta(d => d + 1); setErrors(p => ({ ...p, delta: '' })); }}
                                    className="w-12 h-12 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 flex items-center justify-center font-bold"
                                >
                                    <Plus size={20} />
                                </button>
                            </div>
                            {errors.delta && (
                                <p className="mt-2 text-xs font-semibold text-destructive flex items-center gap-1.5">
                                    <AlertCircle size={14} className="shrink-0" /> {errors.delta}
                                </p>
                            )}
                        </div>

                        {/* Expected stock preview */}
                        ...
                        <div>
                            ...
                            {errors.reason && (
                                <p className="mt-2 text-xs font-semibold text-destructive flex items-center gap-1.5">
                                    <AlertCircle size={14} className="shrink-0" /> {errors.reason}
                                </p>
                            )}
                        </div>
                        ...
                        <div className="flex gap-3 pt-2">
                            <button
                                onClick={onClose}
                                disabled={isSubmitting}
                                className="flex-1 py-3.5 rounded-xl border border-border bg-card text-foreground font-bold hover:bg-muted disabled:opacity-50"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                disabled={isSubmitting}
                                className="flex-1 py-3.5 rounded-xl bg-primary text-primary-foreground font-bold hover:bg-accent flex items-center justify-center gap-2 shadow-md disabled:opacity-70 disabled:cursor-not-allowed"
                            >
