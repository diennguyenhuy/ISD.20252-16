...
import { Plus, Search, Pencil, Trash2, Package, BookOpen, Disc, Tv, FileText, AlertTriangle, ChevronDown, Loader2 } from 'lucide-react';
...
export default function ManagerHomePage() {
  ...
  const { products, loading, fetchProducts, deleteProduct } = useProductManagement();
  ...
  useEffect(() => {
    // SỬA LỖI: Handle floating promise warning
      ...
      <div className="animate-in fade-in duration-500">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1 font-medium">{products.length} products in stock</p>
          </div>
          <button onClick={() => navigate('/manager/products/add')} className="flex items-center justify-center gap-2 bg-primary text-primary-foreground px-5 py-2.5 rounded-xl hover:bg-accent font-bold">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          ...
          <div className="relative min-w-[160px]">
            <select value={filterType} onChange={e => setFilterType(e.target.value)} className={`w-full appearance-none pr-10 cursor-pointer ${inputClass}`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {(['BOOK', 'CD', 'DVD', 'NEWSPAPER']).map(type => {
            ...
            // SỬA LỖI: Thêm fallback `|| Package` để giải quyết TS2786
            const Icon = TYPE_ICONS[type] || Package;

            return (
                <button key={type} onClick={() => setFilterType(filterType === type ? 'ALL' : type)} className={`flex items-center gap-3.5 p-4 rounded-2xl border transition-all duration-200 text-left ${filterType === type ? 'border-primary bg-primary/10 shadow-sm shadow-primary/5' : 'border-border bg-card hover:border-primary/40 hover:bg-muted/30 shadow-sm'}`}>
                  <div className={`p-2.5 rounded-xl shadow-inner ${filterType === type ? 'bg-primary text-primary-foreground' : TYPE_COLORS[type]}`}>
                    <Icon size={18} />
                  </div>
                  <div>
                    <p className={`text-xs font-semibold mb-0.5 ${filterType === type ? 'text-primary' : 'text-muted-foreground'}`}>{TYPE_LABELS[type]}</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tr className="bg-muted/30 border-b border-border">
                <th className="text-left px-5 py-4 font-semibold text-muted-foreground uppercase text-xs">Product</th>
              ...
              {filtered.map(product => {
                ...
                return (
                    <tr key={product.id} className="border-b border-border/50 hover:bg-muted/30 cursor-pointer" onClick={() => navigate(`/manager/products/${product.id}`)}>
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="min-w-0">
                            <p className="text-foreground font-bold line-clamp-1 mb-0.5">{product.title}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3.5">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center justify-end gap-1.5" onClick={e => e.stopPropagation()}>
                          <button onClick={() => setAdjustProduct(product)} className="p-2 text-muted-foreground hover:text-primary rounded-lg transition-colors"><Package size={16} /></button>
                          <button onClick={() => navigate(`/manager/products/edit/${product.id}`)} className="p-2 text-muted-foreground hover:text-amber-500 rounded-lg transition-colors"><Pencil size={16} /></button>
                          <button onClick={() => setConfirmDelete(product.id)} className="p-2 text-muted-foreground hover:text-destructive rounded-lg transition-colors"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                );
        ...
        {confirmDelete && (
            <div className="fixed inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div className="bg-card rounded-3xl p-6 sm:p-8 max-w-sm w-full shadow-2xl border">
                ...
                <p className="text-muted-foreground text-sm mb-8">Are you sure you want to delete this product?</p>
                <div className="flex gap-3">
                  ...
                  <button
                      ...
                      className="flex-1 py-3 rounded-xl bg-destructive text-destructive-foreground font-bold hover:opacity-90 transition-opacity"
                  >
                    Delete
                  </button>
                </div>
